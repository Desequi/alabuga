...
st.set_page_config(layout='wide', page_title='Алабуга')
...
if img_det is not None:
    ...
    st.header('Аугментация')
    col1, col2, col3 = st.columns(3)
    test_dir = col1.text_input('введите путь к датасету', 'vid_input/1')
    tmp_dir = col2.text_input('введите путь назначения', 'vid_input/1')
    col3.write(' ')
    col3.write(' ')
    if col3.button('Аугментировать'):
        ...
        st.success('Датасет из '+test_dir+' аугментирован в '+tmp_dir)
else:
